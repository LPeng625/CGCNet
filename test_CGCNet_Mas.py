...
def test_ce_net_vessel():
    ...
    source = '/home/lpeng/data/Massachusetts/val/images'

    val = os.listdir(source)
    disc = 20
    # 使用多种模型进行测试
    solver = TTAFrame(CGCNet)
    modelName = 'CGCNet-FFM_CSAM-EDM_DS_C1_rerun_4'
    NAME = './weights/M-' + modelName + '.th'
    solver.load(NAME)

    print(NAME)

    # target = '/data/guoxuejun/yangjialin/CE-Net/submits/log_U_Net/'
    target = './submits/M-' + modelName + '/'


    tic = time()
    if not os.path.exists(target):
        os.mkdir(target)
    gt_root = '/home/lpeng/data/Massachusetts/val/masks'
