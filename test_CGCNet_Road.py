...
from networks.BiReNet import CGCNet
...
class TTAFrame():
    ...
    def test_one_img_from_path_4(self, path):
        ...
        # TODO contiguous() 张量在内存中连续
        maska = self.net.forward(img1.contiguous()).squeeze().cpu().data.numpy()
        maskb = self.net.forward(img2.contiguous()).squeeze().cpu().data.numpy()
        maskc = self.net.forward(img3.contiguous()).squeeze().cpu().data.numpy()
        maskd = self.net.forward(img4.contiguous()).squeeze().cpu().data.numpy()
    ...
    def test_one_img_from_path_2(self, path):
        ...
        # TODO contiguous() 张量在内存中连续
        maska = self.net.forward(img5.contiguous()).squeeze().cpu().data.numpy()  # .squeeze(1)
        maskb = self.net.forward(img6.contiguous()).squeeze().cpu().data.numpy()
    ...
    def test_one_img_from_path_1(self, path):
        ...
        # TODO contiguous() 张量在内存中连续
        mask = self.net.forward(img5.contiguous()).squeeze().cpu().data.numpy()  # .squeeze(1)
...
def test_ce_net_vessel():
    ...
    source = '/home/lpeng/data/deepglobe/val/images'

    val = os.listdir(source)
    disc = 20
    solver = TTAFrame(CGCNet)
    # 加载训练模型权重
    modelName = 'D-CGCNet'
    NAME = './weights/' + modelName + '.th'
    solver.load(NAME)
    ...
    target = './submits/' + modelName + '/'
    if not os.path.exists(target):
        os.mkdir(target)

    gt_root = '/home/lpeng/data/deepglobe/val/masks'
    ...
    for i, name in enumerate(val):
        ...
        #     gt_out = np.zeros(shape=np.shape(ground_truth))
        #     gt_out[ground_truth > 0] = 255
        #     cv2.imwrite(target + 'low_' + name.split('.')[0] + '-0.png', img)
        #     cv2.imwrite(target + 'low_' + name.split('.')[0] + '-1.png', gt_out)
        #     cv2.imwrite(target + 'low_' + name.split('.')[0] + '-2.png', mask.astype(np.uint8))
